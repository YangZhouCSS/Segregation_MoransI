...
to setup
 ...
 gis:set-world-envelope gis:envelope-of dc-dataset 
  ...
  ask patches with [centroid? = true] [ set myneighbors n-of 0 patches] ;;empty agentset
  ...
  ask patches with [centroid? = true and ID = x ] [set myneighbors (patch-set myneighbors patches with [centroid? = true and ID = y ]) ]
  ]
  file-close
  ...
  ;;fill in color on map
    foreach gis:feature-list-of dc-dataset
     [if gis:property-value ? "SOC" = "RED" [ gis:set-drawing-color 17  gis:fill ? 2.0]    
      if gis:property-value ? "SOC" = "BLUE" [ gis:set-drawing-color 97  gis:fill ? 2.0]
      if gis:property-value ? "SOC" = "UNOCCUPIED" [ gis:set-drawing-color 7  gis:fill ? 2.0]]
  ...
  while [y <= 188] [
     ...
     if color1 = ["BLUE"] [
       ask n-of (0.4 *(item 0 popu1 / 10)) patches with [ID = y and occupied? = false] [sprout 1 [set tID y set tcolor "RED" set color blue set size 2 ask patch-here[set occupied? true]]]
       ask n-of (0.6 *(item 0 popu1 / 10)) patches with [ID = y and occupied? = false] [sprout 1 [set tID y set tcolor "BLUE" set color red set size 2 ask patch-here[set occupied? true]]]]
...
ask turtles [ask patches with [centroid? = true and ID = [tID] of myself] [ask myself [set tneighborpolygons [myneighbors] of myself]]
             let tneighborpolygons1 count tneighborpolygons
             ifelse color = red [set blueratio-polygons count tneighborpolygons with [mycolor = "BLUE"] / tneighborpolygons1 ]
                    [set redratio-polygons count tneighborpolygons with [mycolor = "RED"] / tneighborpolygons1 ]
]

ask turtles [ let neighbors1 count turtles-on neighbors
              ifelse neighbors1 > 0 [
                 ifelse color = red [set blueratio count (turtles-on neighbors) with [color = blue] / neighbors1]
                    [set redratio count (turtles-on neighbors) with [color = red] / neighbors1]]
              [set redratio 0 set blueratio 0]
]

;;decide if they are happy
ask turtles with [color = red] [ifelse blueratio < Percentage-different-to-be-unhappy / 100 and blueratio-polygons < Percentage-different-to-be-unhappy / 100 [set happy? true][set happy? false]]

ask turtles with [color = blue] [ifelse redratio < Percentage-different-to-be-unhappy / 100 and redratio-polygons < Percentage-different-to-be-unhappy / 100 [set happy? true][set happy? false]]
...
to update-colors   ;;update polygon colors
 ...
 ask patches with [centroid? = true][
   ...
   ifelse count total-here with [color = red] > (0.5 * count total-here) [set mycolor "RED"][ set mycolor "BLUE"]
   ][set mycolor "UNOCCUPIED"]
   

   if mycolor = "RED" [ gis:set-drawing-color 17  gis:fill  item (ID - 1) gis:feature-list-of dc-dataset 2.0]    
   if mycolor = "BLUE" [ gis:set-drawing-color 97  gis:fill  item (ID - 1) gis:feature-list-of dc-dataset 2.0]
   if mycolor = "UNOCCUPIED" [ gis:set-drawing-color 7  gis:fill  item (ID - 1) gis:feature-list-of dc-dataset 2.0]
   ]
  gis:set-drawing-color white
  gis:draw dc-dataset 1
end
...
to get-pctred
  ...
  ask patches with [centroid? = true][ifelse mycolor = "UNOCCUPIED" [set pctred 0.5][set pctred redt / (redt + bluet)] 
  ;ask patches with [centroid? = true][if mycolor = "UNOCCUPIED" [set pctred 0.5] if mycolor = "RED" [set pctred 1] if mycolor = "BLUE" [set pctred 0] ;;alternative way
